...
public class UpdateDiscountHandler(ICouponRepository couponRepository):IRequestHandler<UpdateDiscountCommand,CouponDto>
{
    public async Task<CouponDto> Handle(UpdateDiscountCommand request, CancellationToken cancellationToken)
    {
        ...
        var updated = await couponRepository.UpdateDiscount(coupon);
        return !updated ? throw new RpcException(new  (StatusCode.Internal,$"Could not update discount for product {request.ProductName}")) : coupon.ToDto();
    }
}